...
def split_train_test(X: pd.DataFrame, y: pd.Series, train_proportion: float = .75) \
    ...
    """
    num_train_rows = int(np.ceil(train_proportion*len(X))) #ceil(train_proportion * n,samples)
    train_indices = X.sample(n=num_train_rows, random_state=0).index
    train_X = X.iloc[train_indices]
    train_y = y.iloc[train_indices]
    test_X = X.iloc[~X.index.isin(train_indices)]
    test_y = y.iloc[~y.index.isin(train_indices)]
